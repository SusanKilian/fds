...
INTEGER, PARAMETER :: NSCARC_STORAGE_CSR             =  0, &    !> CSR storage type
                      ...
                      NSCARC_STORAGE_CONDENSED       =  2, &    !> Condensed storage type
                      NSCARC_STORAGE_SWAP            =  3, &    !> Swapping storage type
                      NSCARC_STORAGE_FREE            =  4       !> Free storage type
...
CHARACTER(80) :: MKL_ERROR_PARDISO, MKL_ERROR_CLUSTER, MKL_ERROR_STORAGE, FFT_ERROR_UNSTRUCTURED
...
MKL_ERROR_STORAGE      = 'Error: PARDISO and Cluster_Sparse_Solver only available for CSR storage technique'
...
!>
!> ------------ set type of matrix storage (CSR/BANDED)
!>
SELECT CASE (TRIM(SCARC_STORAGE))
   ...
   CASE DEFAULT
      CALL SCARC_SHUTDOWN('Error with input parameter ', SCARC_STORAGE, -999)
END SELECT
         ...
         CASE ('MULTIGRID')
            ...
            SELECT CASE (TRIM(SCARC_SMOOTH))
               ...
               CASE ('PARDISO')
                  ...
                  ELSE
                     CALL SCARC_SHUTDOWN(MKL_ERROR_STORAGE, 'NONE', -999)
                  ENDIF
#else
   ...
   CASE (NSCARC_METHOD_MULTIGRID)
      ...
      SELECT_MULTIGRID: SELECT CASE (TYPE_MULTIGRID)
         ...
         CASE (NSCARC_MULTIGRID_ALGEBRAIC)
            ...
            WRITE(*,*) 'SCARC_SETUP_LEVELS: TO FIX'
...
INTEGER  :: IW, IWL, IWG, IWW, IW0, IROW, ICOL, IPTR, ICPL
INTEGER  :: IOR_NBR, IOR_OWN, IC, JC, ICE, ICN, ICG, ICW, IFACE
INTEGER  :: IX, IY, IZ
INTEGER  :: IXW, IYW, IZW
INTEGER  :: IXG, IYG, IZG
!INTEGER  :: IXN, IYN, IZN
         ...
         CASE (NSCARC_EXCHANGE_MATRIX_SYSTEM)
            ...
            PACK_MATRIX_SYSTEM: DO ICG=1, OL%NCG
               ...
               DO ICPL = 1, OL%NCPLR
                  ...
                  MATRIX_COLUMN_LOOP: DO ICOL = A%CSR%ROW(IC), A%CSR%ROW(IC+1)-1
                     JC = A%CSR%COL(ICOL)
                     IF (JC > L%NC) THEN
                        IWL = M%ICE_TO_IWL(JC)
                     ...
                     ENDIF
                     OX%SEND_REAL(LL+1) = A%CSR%VAL(ICOL)
                     LL = LL + 2
         ...
         OMESH_UNPACK_SELECT: SELECT CASE (TYPE_EXCHANGE)
            ...
            CASE (NSCARC_EXCHANGE_MATRIX_SUBDIAG)
               ...
               UNPACK_MATRIX_SUBDIAG: DO IWL = 1, OL%NWL
                  ...
                  IROW = A%CSR%ROW(IC)
                  DO ICOL = A%CSR%ROW(IC)+1, A%CSR%ROW(IC+1)-1
                     IF (IW == -A%CSR%COL(ICOL)) THEN
                        A%CSR%VAL(ICOL)     = ZSUM/REAL(L%WALL(IW)%NCPL,EB)
                        A%CSR%COL(ICOL) = L%WALL(IW)%ICE(1)
                     ENDIF
                  ENDDO
            ...
            CASE (NSCARC_EXCHANGE_MATRIX_SYSTEM)
               ...
               UNPACK_MATRIX_SYSTEM: DO IWL = 1, OL%NWL
                  ...
                  DO ICOL = A%CSR%ROW(ICE), A%CSR%ROW(ICE+1)-1
                     A%CSR%COL(ICOL)= NINT(RECV_REAL(LL))
                     A%CSR%VAL(ICOL)    = RECV_REAL(LL+1)
                     LL = LL + 2
                  ENDDO
...
SELECT_STORAGE_TYPE: SELECT CASE(TYPE_STORAGE)
   ...
   CASE(NSCARC_STORAGE_CSR)
      ...
      ELSE
         CALL SCARC_SHUTDOWN('SCARC_RESIZE_MATRIX: New matrix length too big ', 'NONE', NSIZE)
      ENDIF
